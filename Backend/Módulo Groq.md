...
- [[Kmd]] (`_cmdKneAi`) va reescribiendo la misma línea de espera con el texto acumulado a medida que streamea, en vez de esperar la respuesta completa (2026-09-02). El `context` acá le pasa además la tabla de comandos de Kmd y la instrucción de responder `#run <comando>` cuando el pedido se resuelve ejecutando uno — `_cmdKneAi` intercepta ese formato y corre el comando de verdad en vez de solo mostrar texto (ver [[Kmd#`kneai` conoce los comandos y puede ejecutarlos (2026-09-02)|Kmd]]).
