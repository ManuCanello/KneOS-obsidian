...
- **`saveFolderStyle(folder_id, pc_id, folder_view, folder_group_by, folder_group_order)`** *(2026-07-28: firma cambiada + reescrito a upsert atómico)*: primero una sonda de ownership, `icons.findFirst({where:{id_icon: folder_id, pc_id}})` — si no existe, devuelve `null` sin tocar `folder_styles`. Si pasa, `prisma.folder_styles.upsert({where:{folder_id}, update, create})` — ahora sí un `upsert` nativo de Prisma, porque `folder_id` tiene `@unique` en el schema (ver nota de condición de carrera abajo).
