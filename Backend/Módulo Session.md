...
- **`verificarSesion(req, res)`**: lee el token de `req.cookies` (`readSessionToken`, ver [[Backend#middlewares]]). Si el token no es válido **o** `existeSesion(pcId)` da falso, `401 { error: "Sesión inválida" }`. Si pasa ambos, renueva la cookie (mismo `pcId`, vencimiento nuevo) y responde `{ success: true }`.
