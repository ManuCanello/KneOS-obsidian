...
- **`deleteIconRecursivo(tx, id_icon, pc_id)`** *(interna, 2026-07-27)*: busca los hijos directos (`parent_id`, scoping por `pc_id`) y se llama a sí misma sobre cada uno **antes** de tocar el ícono actual (post-order, hijos antes que el padre — necesario porque `icons_icons_fk` es `onDelete: NoAction`); recién entonces borra su `txt`, su `camera_photos` (2026-08-19, ver [[Módulo CameraPhoto]]), su `kpaint_files` (2026-08-21, ver [[Módulo KPaint]] — mismo `onDelete: NoAction` que `camera_photos`), su `folder_styles` (ver [[Módulo Folder Styles]]) y el propio `icon`.
  ...
  > [!bug] `kpaint_files` faltaba en la recursión (agregado 2026-08-20 con KPaint, encontrado y corregido 2026-08-21)
  > `kpaint_files` tiene la misma FK `onDelete: NoAction` que `camera_photos` (ver `prisma/schema.prisma`), pero al agregar KPaint no se sumó su `deleteMany` acá — quedó el borrado de `txt`/`camera_photos`/`folder_styles` pero no el de la tabla nueva. Resultado: vaciar la papelera con un `kp` adentro tiraba `P2003` (foreign key constraint violated) al llegar a `tx.files.delete`, y ese `kp` quedaba atascado en la papelera para siempre (el purge fallido no lo saca de la lista). Fix: `await tx.kpaint_files.deleteMany({ where: { id_icon } })` junto al resto, mismo lugar/orden que `camera_photos`. Verificado creando un `kp` desde KPaint, mandándolo a la papelera y vaciándola — `/icons/purge` pasó de 500 a 200.
