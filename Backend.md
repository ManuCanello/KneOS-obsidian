...
3. Monta 23 routers: `/groq`, `/session`, `/kneAI`, `/iconRoutes`, `/kfruitRoutes`, `/hangmanRoutes` (2026-08-12, ex `/ahorcadoRoutes`, renombrada 2026-08-13), `/flipcoinRoutes` (2026-08-12), `/kdleRoutes` (2026-08-12, ex `/wordleRoutes`), `/vroomRoutes` (2026-08-13, ex `/carreraautoRoutes`→`/carRaceRoutes`, renombrada de nuevo a `/vroomRoutes` el 2026-09-18 — ver [[Módulo VROOM]]), `/tetrisRoutes` (2026-08-13), `/txtRoutes`, `/cameraPhotoRoutes` (2026-08-19, ver [[Módulo CameraPhoto]]), `/kpaintRoutes` (2026-08-20, ver [[Módulo KPaint]]), `/folderGroupByRoutes`, `/folderStylesRoutes`, `/folderViewsRoutes`, `/chatRoutes` (2026-08-18, ver [[Módulo Chat]]), `/themeRoutes` (2026-08-19, ver [[Módulo Theme]]), `/mapRoutes` (2026-08-21, ver [[Módulo Map]]), `/spotifyRoutes` (2026-08-24, ver [[Módulo Spotify]]), `/guestbookRoutes` (2026-08-31, ver [[Módulo Guestbook]]), `/trucoRoutes` (2026-09-03, ver [[Módulo Truco]]), `/cameraViewRoutes` (2026-09-15, ver [[Módulo CameraView]]).
...
## `realtime/` — tiempo real (2026-08-18, segundo archivo 2026-09-03)

Capa fuera de la cadena clásica `routes/ → controllers/ → models/`: no sirve HTTP, solo empuja eventos a sockets ya conectados. `chatHub.js` — WebSocket de [[KneChat]], detalle completo en [[Módulo Chat#`realtime/chatHub.js` — WebSocket]] — y `trucoHub.js` (2026-09-03) — WebSocket del modo ONLINE de [[Truco]], ver [[Módulo Truco]]. Los dos autentican el handshake reusando `readSessionToken()` de `middlewares/auth.js` (mismo mecanismo que HTTP, sin sistema de auth propio para el socket); escribir/leer sigue siendo HTTP normal contra su router, que dispara el broadcast recién después de persistir (o, en el caso de Truco, después de validar quién puede pushear qué — ver [[Módulo Truco]]).

Los dos hubs comparten el mismo `http.Server` (`server.js`), enganchados sobre el mismo evento `'upgrade'` — eso obliga a una bandera compartida (`socket.__kneosWsClaimed`) y un catch-all único al final de `server.js`, en vez de que cada hub destruya por su cuenta lo que no matchea su propio path (ver [[Módulo Truco#Bug real encontrado y corregido: `server.on('upgrade')` con dos hubs]]).
...
|--------------------|-----------------|-------------------------------------------------------------------------------|------------|
| `sessions`         | `pc_id`         | `creation_date`, `nickname` (2026-08-18, único, nullable — alias de [[KneChat]], no una cuenta) | 1–N con `kneai_chats`, `kneai_messages`, `chat_room_members`, `chat_messages`; 1–1 con `kfruit_keybinds`, `tetris_keybinds`, `theme_settings`, `camera_view_state` |
...
| `vroom_results` (2026-08-13, ex `car_race_results`/`carreraauto_resultados`, renombrada 2026-09-18) | `id_result` | `car` (VarChar, `rojo`/`azul`/`cian`/`blanco` — el valor no se tradujo, solo la columna), `premio` (Int, default 0, agregada 2026-09-18 — lo que pagó esa fila al auto ganador, ver [[Módulo VROOM]]), `created_at` | ninguna — log global sin `pc_id`, crece con cada carrera, mismo criterio que `flipcoin_results` |
...
| `guestbook_signatures` (2026-08-31) | `signature_id` | `pc_id` (`@unique`), `message` (VarChar 280), `strokes` (Json — polilíneas normalizadas 0..1, nunca un PNG), timestamps | N–1 con `sessions` — calco de `theme_settings` (upsert por `pc_id`, una fila por sesión), ver [[Módulo Guestbook]] |
...
| `truco_matches` (2026-09-03) | `match_id` | `mode` (enum `truco_mode`: `con_flor`\|`sin_flor`\|`v2`), `host_pc_id`, `guest_pc_id`, `host_score`, `guest_score`, `winner_pc_id?` (VarChar suelto, no FK — siempre igual a uno de los dos `pc_id` de arriba), `created_at`, `finished_at?` | Dos relaciones NOMBRADAS con `sessions` (`truco_matches_host`/`truco_matches_guest` — dos FKs distintas al mismo modelo necesitan nombre); log de resultados, no estado de juego — mismo criterio que `vroom_results`, ver [[Módulo Truco]] |
| `camera_view_state` (2026-09-15) | `id_state` | `pc_id` (`@unique`), `x`/`y`/`z`/`target_x`/`target_y`/`target_z` (Float, posición+target de `OrbitControls`), `modo?` (`"pc"`\|`"guestbook"`\|null), `dist?` (Float, distancia de dolly del foco activo) | N–1 con `sessions` — calco de `theme_settings` (upsert por `pc_id`), pero sin default: sin fila, el frontend cae a sus propias constantes, ver [[Módulo CameraView]] |
...
- [[Módulo VROOM]] — log de carreras y cuotas del minijuego de apuestas a autos (ex CarRace)
...
- [[Módulo Guestbook]] — libro de firmas 3D: mensaje + trazos vectoriales, una firma por sesión
- [[Módulo Truco]] — modo ONLINE de Truco: lobby/invitaciones/relay de partida (anfitrión autoritativo) e historial de ganadas/perdidas por modo
- [[Módulo CameraView]] — última posición/target/modo de la cámara 3D de la escena principal (`public/js/main.js`), una por sesión
