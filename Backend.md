...
| `truco_matches` (2026-09-03) | `match_id` | `mode` (enum `truco_mode`: `con_flor`\|`sin_flor`\|`v2`), `host_pc_id`, `guest_pc_id`, `host_score`, `guest_score`, `winner_pc_id?` (VarChar suelto, no FK — siempre igual a uno de los dos `pc_id` de arriba), `created_at`, `finished_at?` | Dos relaciones NOMBRADAS con `sessions` (`truco_matches_host`/`truco_matches_guest` — dos FKs distintas al mismo modelo necesitan nombre); log de resultados, no estado de juego — mismo criterio que `car_race_results`, ver [[Módulo Truco]] |
