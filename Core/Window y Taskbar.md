...
- `_hacerMovible` — configura `interact(ventana).resizable(...)` (bordes, mínimo 300×200, bloquea overlay durante resize — **solo si `_permiteResize`**) e `interact(barra).draggable(...)` siempre, con la lógica de snap descrita abajo (la detección de zona de snap dentro del `move` también se salta si `!_permiteResize`, así `activeZone` nunca se setea y el `end` nunca aplica un rect de snap). El listener `start` del resizable corta la interacción con `event.interaction.stop()` si la ventana está maximizada (`this._maximizada`, es decir `_snapZone === "full"`): una ventana maximizada no se puede redimensionar arrastrando bordes, hay que restaurarla primero (botón o arrastre de la barra) para volver a habilitar el resize.
...
> [!info] Intento de implementación revertido (2026-08-21)
> El 2026-08-21 se conectaron estos dos ítems a un módulo nuevo `core/PantallaEnergia.js` (arranque/login/apagado simulados dentro del iframe, con animación CRT tipo terminal y gate de login usuario `Kne`/contraseña hardcodeada) — funcionando y verificado con Playwright, pero se revirtió el mismo día a pedido explícito del usuario ("elimina todo lo del boot, lo retomaré más adelante"). El plan de diseño completo (estado en `localStorage`, estructura del módulo, wiring en `KNEOS.js`/`Home.js`, CSS) queda guardado en `C:\Users\canel\.claude\plans\quiero-seguir-con-el-binary-cosmos.md` para no tener que re-investigar desde cero la próxima vez que se retome.
