...
`run ahorcado|kdle|blackjack|flipcoin|tetris|vroom` juega uno de los seis puertos de Java **adentro de la terminal**, a pedido explícito del usuario ("no crear archivos nuevos, reutilizar los que están"). No abre una `Window` nueva: `RUN_GAMES` mapea el alias a un `ext` real de `iconsSrc` (ver [[Frontend Model Services Utils]] — hasta 2026-09-18 el alias era `carrera`, único que no coincidía 1:1 con el `ext` real (`"carreraauto"`); con el rename a VROOM el alias pasó a `vroom` y ahora coincide con el `ext` como el resto, ya no hace falta el caso especial), `_cmdRun` usa `loadAppClass(ext)` (el mismo lazy-import que ya usa `DesktopManager`, ver [[DesktopManager]]) para traer la clase, instancia `new Clase(alias)` "pelada" (sin pasar por `abrirVentana()`/`Window` real — ver [[File]], confirmado seguro) y monta directo lo que devuelve `instancia._crearContenido()` en un `div.kmdRunContainer` que reemplaza a `.kmdOutput` mientras dura (mismo alto disponible, `flex:1;min-height:0`, ver `kmd.css`).
