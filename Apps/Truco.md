...
- `_cerrarRonda2()`: puntaje con hasta tres multiplicadores/bonus (presión x2, carta de poder "Doble apuesta" x2, rol Apostador +1 si ganó con Truco cantado), archiva las 3 cartas ELEGIDAS para la mano en `_usadas` (`{carta, motivo}`: `"jugada"` para las que de verdad se jugaron, `"elegida"` para las que no llegaron a jugarse esa ronda, `"desgaste"` para las descartadas en `_desgasteMazo2` -- tres motivos, no dos, para que el Conservador siga recuperando solo cartas realmente jugadas). También lleva la racha de victorias consecutivas (`_racha`) para el objetivo "2 rondas seguidas" y llama a `_evaluarObjetivo2()`.
