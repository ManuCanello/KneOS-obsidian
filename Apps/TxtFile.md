...
- **Botón de IA general (`.txtIconAi`, 2026-07-30)**: ícono `sources/appIcon/kneAi.png` aplicado con `aplicarIconoImagen` (PNG ya coloreado, sin mask). A diferencia de los demás `.txtIcon`, anula el hover-invert de fondo (el ícono es verde y quedaría invisible contra el fondo verde del invert) y usa `filter: brightness(1.3)` como señal de hover en su lugar.
  ...
  - **Flujo de dos vistas dentro del mismo popup** (no toca la nota hasta que el usuario confirma):
    ...
    - **`async _askAi(popup, textarea)`**: arma el mismo tipo de prompt de sistema que [[KneAI]] (fuerza HTML puro, nada de Markdown). **Streaming (2026-09-02):** arma la vista de preview (`.txtAiPreview`, vacía) de entrada, antes de tener respuesta, y llama `window.groq.ask(context, question, [], onChunk)` — sin historial, cada pregunta es independiente. El callback pisa `preview.innerHTML = full` en cada delta, así el usuario ve el texto aparecer en vivo en vez de esperar en blanco. Si falla (`null`), vuelve a `_renderAskView` con la pregunta puesta para reintentar (antes se quedaba en la vista de pregunta sin más; ahora hay que volver explícitamente porque la preview ya se había mostrado).
    - **`_renderAnswerView(popup, question, preview, answer)`** *(2026-09-02: recibe `preview` ya armado por `_askAi`, no lo crea)*: fija el HTML final (`preview.innerHTML = answer`, por si el último delta hubiera quedado corto) y agrega la fila de acciones (`.txtAiActions`) con **Aceptar** (inserta la respuesta y cierra el popup), **Volver** (vuelve a `_renderAskView` con la pregunta restaurada, sin descartar nada) y **Cancelar** (cierra el popup sin tocar la nota). Los tres `click` hacen `stopPropagation` — necesario porque mutan `popup.innerHTML` sincrónicamente, y si el click siguiera burbujeando hasta el listener de "click afuera" de `document`, el botón ya estaría desconectado del popup y el listener lo interpretaría como un click afuera, cerrando todo de golpe (bug real, encontrado y arreglado en la vuelta "Volver").
...
- **IA de selección — menú contextual (2026-07-30)**. Pasó por dos diseños antes de este: primero un botón fijo bajo la toolbar, después un botón flotante que seguía la selección (`.txtAiSelectionFloater`, con `getBoundingClientRect()` para no depender de que las ventanas se arrastran con CSS `transform`, ver [[Window y Taskbar|Window]]) — descartado por el usuario ("no funciona correctamente") en favor de un menú contextual nativo del proyecto:
  ...
  - **`async _askSelectionAi(input, range)`**: arma un prompt distinto al general (reescribe un fragmento dado + una instrucción, en vez de responder libremente) con el mensaje `Texto seleccionado: "..." / Instrucción: ...`. **Streaming (2026-09-02):** ya no arma el reemplazo recién al final — saca el contenido seleccionado con `range.extractContents()` (no `deleteContents()`: devuelve un `DocumentFragment` con lo sacado, necesario para poder restaurarlo), deja un `<span>` placeholder vacío en su lugar (`range.insertNode`), y el callback de `window.groq.ask(...)` pisa `placeholder.innerHTML = full` en cada delta — el reemplazo se ve crecer en vivo en el lugar de la selección original. Si falla (`null`), `placeholder.replaceWith(original)` devuelve el fragmento extraído tal cual estaba, sin pérdida de texto (antes no hacía falta este paso porque nada se tocaba hasta tener la respuesta completa).
