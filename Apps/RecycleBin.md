...
> [!bug] Ciclo de imports con `iconSrc.js` (2026-07-31, resuelto el mismo día)
> `iconSrc.js` importa **estáticamente** la clase `RecycleBin` (para el registro `recyclebin: {css, Class}`, igual que hace con las otras 7 apps). La primera versión de esta clase también importaba `getIconMeta` de `iconSrc.js` arriba de todo (estático, para resolver el ícono de cada fila por extensión) — eso cierra un ciclo `iconSrc.js → RecycleBin.js → iconSrc.js`. A diferencia de las otras apps (que no le importan nada a `iconSrc.js`), acá el ciclo rompía en runtime: `Uncaught ReferenceError: Cannot access 'RecycleBin' before initialization` en `iconSrc.js`, porque cuando `iconSrc.js` llega a armar `iconsSrc` (línea `recyclebin: {..., Class: RecycleBin}`), todavía está en medio de resolver su propio import de `RecycleBin.js`, que a su vez quedó a mitad de camino esperando el import circular de vuelta a `iconSrc.js` — la clase `RecycleBin` nunca llegó a inicializarse (TDZ) antes de que `iconSrc.js` intentara leerla. Fix: sacar el `import { getIconMeta } from "../model/iconSrc.js"` estático y reemplazarlo por un `import()` **dinámico** dentro de `_cargarPapelera()`, resuelto (y pasado como parámetro a `_agregarFila`) recién cuando el usuario abre la ventana — para ese momento el grafo de módulos ya terminó de cargar por completo, así que no hay ciclo que romper.
