...
- **Toolbar** (`recycleBinToolbar`/`.recycleBinToolbarBtn`, `recyclebin.css`): un único botón "Vaciar papelera" → `_vaciarPapelera()`, alineado a la derecha (`.recycleBinToolbar { justify-content: flex-end; }`, 2026-09-16) con el ícono de `sources/appIcon/trash.svg` (`.recycleBinToolbarBtnIcon`, 16px, mismo `aplicarIconoImagen` que el resto de los botones con ícono — ver `iconoStyle.js`) a la izquierda del texto.
- **Header de lista** (reusa `.folderListaHeader`/`.folderListaHeaderCol` de `folder.css`, siempre visible — no hay otra vista): columnas Nombre / **Fecha Elim** (2026-09-16, ex "Fecha de eliminación" — mismo acortamiento que `folder.css` le hizo a la columna de fecha de `Folder`) / Tipo / **Ubicación original** (columna nueva, `.iconoColumna--ubicacion`, `recyclebin.css`) / Tamaño.
...
- **`confirmarVaciarPapelera(x, y)`** (2026-07-31): el punto de entrada real para el usuario, tanto desde el botón de la toolbar (`e.clientX/clientY` del click) como desde el ítem "Vaciar papelera" del menú contextual del propio ícono (ver [[Menús Contextuales]], posicionado con el `getBoundingClientRect()` de ese ítem). Abre un `ContextMenu` más (`CONFIRMAR_VACIAR_MENU_ID`) con dos opciones — **"Sí, vaciar la papelera"** (llama `vaciarPapelera()`) y **"Cancelar"** (no hace nada) — en vez de un `window.confirm()` nativo: no hay ningún `confirm()`/`alert()` nativo en todo el proyecto, porque rompería la estética retro monocromática (ver [[Reglas]]); un `ContextMenu` más mantiene el mismo idioma visual que ya usa el resto del sistema para cualquier decisión del usuario.
  > [!info] Tamaño propio y mensaje más grande (2026-09-16)
  > `460×280` (antes `420×250`) y `mensajeClase: "alertWindowMensaje--grande"` — nuevo parámetro opcional de `AlertWindow` (`core/AlertWindow.js`, default `""`) que suma una clase extra al `<p>` del mensaje sin tocar el resto de los llamadores (KneChat/KPaint siguen en el `font-size: 20px` por defecto de `.alertWindowMensaje`, `core/alert.css`). `.alertWindowMensaje--grande` sube a `24px`.
